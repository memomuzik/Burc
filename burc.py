...
async def etiketle(event):
    ...
    mention1 = random.choice(users).(f'[{event.sender.first_name}](tg://user?id={event.sender.id})')
    mention2 = random.choice(users).(f'[{event.sender.first_name}](tg://user?id={event.sender.id})')
